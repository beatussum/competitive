    ...
    for (; t != 0; --t) {
        ...
        for (auto i = test.cbegin(); i < test.cend(); ) {
            switch (*i) {
                ...
                case '1': {
                    std::string::const_iterator first = i++, last;

                    for (;;) {
                        i = std::find(i, test.cend(), '0');

                        if (i != test.cend()) {
                            i = last = std::find(i, test.cend(), '1');
                        } else {
                            if (last != std::string::const_iterator()) {
                                test.replace(first, last, 1, '0');
                            }
