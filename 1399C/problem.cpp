...
using test_t = std::vector<me::uint_fast8_t>;
...
int main()
{
    ...
    for (const auto& test : tests) {
        me::uint_fast8_t max_teams = 0;

        for (me::uint_fast8_t w = 100; w >= 2; --w) {
            std::vector<test_t::const_iterator> sel;
            sel.reserve(test.size() / 2);

            for (auto i = test.cbegin(); i != test.cend(); ++i) {
                if (std::find(sel.cbegin(), sel.cend(), i) != sel.cend()) {
                    continue;
                }

                for (auto j = i + 1; j != test.cend(); ++j) {
                    if (std::find(sel.cbegin(), sel.cend(), j) != sel.cend()) {
                        continue;
                    }
            ...
            if ((sel.size() / 2) > max_teams) {
                max_teams = sel.size() / 2;
            }
        }
