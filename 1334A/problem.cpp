...
int main()
{
    ...
    for (; t != 0; --t) {
        ...
        std::uint_fast32_t pold = 0, cold = 0;

        for (; n != 0; --n) {
            std::uint_fast32_t p, c;
            std::cin >> p >> c;

            const bool is_decreasing = (p < pold) || (c < cold);
            const bool is_coherent = (c > cold) ? (p - pold) >= (c - cold) : true;

            if (is_decreasing || (c > p) || !is_coherent) {
                std::cout << "NO\n";
