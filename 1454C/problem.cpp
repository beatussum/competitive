...
using vector = std::vector<std::uint32_t>;

template <class _ForwardIt>
_ForwardIt find_first_not_of(const _ForwardIt __begin, const _ForwardIt __last)
{
    return std::min(
        std::adjacent_find(__begin, __last,
            [&] (const auto __l, const auto __r) {
                return ( __l != __r ) && ( __l == *__begin );
            }
        ) + 1,
        __last
    );
...
int main()
{
    ...
    for (; t != 0; --t) {
        ...
        m[v.front()] = m[v.back()] = 0;

        for ( auto i = v.cbegin()
            ; ( i = find_first_not_of(i, v.cend()) ) != v.cend()
            ;)
        {
            m[*i] += 1;
        }
        ...
        if (m.size() == 1) {
            ...
            min = std::min_element(m.cbegin(), m.cend(),
                [] (const auto& __l, const auto& __r) {
                    return __l.second < __r.second;
                }
            )->second + 1;
        }
