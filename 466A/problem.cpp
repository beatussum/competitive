...
int main()
{
    std::uint_fast16_t n, m, a, b;
    std::cin >> n >> m >> a >> b;

    auto min = std::numeric_limits<std::uint_fast32_t>::max();

    for (std::uint_fast16_t x = 0; x <= n; ++x) {
        for (std::uint_fast16_t y = 0; y <= (n / m) + 1; ++y) {
            const std::uint_fast32_t price = a * x + b * y;
            const std::uint_fast32_t rides = x + m * y;

            if ((rides >= n) && (price < min)) {
                min = price;
            }
        }
    }
